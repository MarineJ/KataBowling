...
class CalculScore():
	...
	# two matrix are filled, one with int in range(0,3) for bonus counting when strike or spare, the second one is for storing rolls scores
	def updateValues(self, i, val1, val2, isOnePrevStrike, are2PrevStrikes):
		if are2PrevStrikes:
			self.LogicalStrikeSpare[i-1,range(0,4)]=[1,1,2,1]
		elif isOnePrevStrike:
			self.LogicalStrikeSpare[i-1,range(0,4)]=[1,1,1,1]
		if i<10:
			if int(val1)==10:
				self.LogicalStrikeSpare[i,range(0,4)]=[1,1,1,1]
